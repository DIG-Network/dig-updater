...
//!
//! Scans `src/` only, matching the reference this was ported from -- NOT `tests/`. This
//! crate's own `tests/scheduler.rs` carries a byte-identical `icacls` fixture and
//! `tests/nightly_release_workflow_shape.rs` had one real hit (fixed directly, outside
//! this guard's scan scope, dig_ecosystem#3130), neither of which this scan ever sees.
#![cfg(test)]
...
/// No fixture in `src/` pins byte-identical captured external output that needs a
/// line-range carve-out (the crate's one such fixture, `tests/scheduler.rs`, lives
/// outside this guard's `src/`-only scan scope).
const EXCLUDED_LINE_RANGES: &[(&str, u32, u32)] = &[];

/// No file in `src/` has adopted the "hand-aligned `\n`-joined banner" idiom. Once a

...
//! (RT_GROUP_ICON/RT_ICON), into the `dig-updater-feedsign` binary. The
//! workspace's `.cargo/config.toml` already embeds an `asInvoker` RT_MANIFEST
//! for every Windows binary via linker rustflags (`/MANIFEST:EMBED` +
...
/// Compile the branded DIG icon into the `dig-updater-feedsign` binary.
///
